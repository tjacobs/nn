...
#
# 13: Adam
# Use momentum and adaptive Adam optimizers
...
def main():
    ...
    if use_momentum:
        velocities_w = [np.zeros_like(w) for w in p_weights]
        velocities_b = [np.zeros_like(b) for b in p_biases]

    # If we are using AdamW
    if use_adamw:
        m_w = [np.zeros_like(w) for w in p_weights]
        v_w = [np.zeros_like(w) for w in p_weights]
        m_b = [np.zeros_like(b) for b in p_biases]
        v_b = [np.zeros_like(b) for b in p_biases]
    ...
    for epoch in range(epochs + 1):
        ...
        for index in range(0, data_len, batch_size):
            ...
            for layer in range(len(p_weights)):
                if use_momentum:
                    velocities_w[layer] = momentum_beta * velocities_w[layer] + (1 - momentum_beta) * learning_rate * gradients_w[layer]
                    velocities_b[layer] = momentum_beta * velocities_b[layer] + (1 - momentum_beta) * learning_rate * gradients_b[layer]
                    p_weights[layer] -= velocities_w[layer]
                    p_biases[layer] -= velocities_b[layer]
                elif use_adamw:
                    m_w[layer] = beta1 * m_w[layer] + (1 - beta1) * gradients_w[layer]
                    v_w[layer] = beta2 * v_w[layer] + (1 - beta2) * (gradients_w[layer] ** 2)
                    m_w_hat = m_w[layer] / (1 - beta1 ** (epoch + 1))
                    v_w_hat = v_w[layer] / (1 - beta2 ** (epoch + 1))
                    m_b[layer] = beta1 * m_b[layer] + (1 - beta1) * gradients_b[layer]
                    v_b[layer] = beta2 * v_b[layer] + (1 - beta2) * (gradients_b[layer] ** 2)
                    m_b_hat = m_b[layer] / (1 - beta1 ** (epoch + 1))
                    v_b_hat = v_b[layer] / (1 - beta2 ** (epoch + 1))
                    p_weights[layer] -= learning_rate * m_w_hat / (np.sqrt(v_w_hat) + epsilon) + weight_decay * p_weights[layer]
                    p_biases[layer] -= learning_rate * m_b_hat / (np.sqrt(v_b_hat) + epsilon) + weight_decay * p_biases[layer]
                else:
